...
class FunctionMetrics {
    ...
    logFunctionStart(isRAMTrackingEnabled, ramTrackInterval, customResourceUsageFactor) {
        ...
        try {
            ...
            var invocationsTotalMetric = generateMetric(FUNCTION_INVOCATIONS_TOTAL, this.defaultLabels, 1) + '\n';
    ...
    logFunctionEnd() {
        ...
        try {
            ...
            var executionTimeMetric = generateMetric(FUNCTION_EXECUTION_TIME_TOTAL, this.defaultLabels, startEndDiffDecimal) + '\n';
            this.metadataManager.log(executionTimeMetric);
            if (!connectionError) {
                this.metricSender.sendMetric(executionTimeMetric);
            }

            var finishedMetric = generateMetric(FUNCTION_FINISHED_TOTAL, this.defaultLabels, 1) + '\n';
            ...
            if (this.isRAMTrackingEnabled) {
                ...
                var gbSecondMetric = generateMetric(FUNCTION_GIGABYTE_SECONDS_TOTAL, this.defaultLabels, gbSeconds);
    ...
    logCustomMetric(metricName, metricValue, additionalLabels, ignoreDefaultLabels) {
        ...
        const metric = generateMetric(metricName, labels, metricValue);
 ...
 */
function generateMetric(metricName, labels, value) {
