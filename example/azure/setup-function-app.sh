...
releaseSAS=$(az storage blob generate-sas --connection-string $storageConnectionString --container-name function-releases --name $fileName | cut -d '"' -f2)

az functionapp config appsettings set --resource-group $resourceGroup --name $functionAppName --settings WEBSITE_NODE_DEFAULT_VERSION=$nodeVersion WEBSITE_RUN_FROM_PACKAGE=1 PROMETHEUS_AGGREGATOR_ADDRESS=$prometheusAggregatorAddress PUSHGATEWAY_ADDRESS=$pushgatewayAddress  COSMOSDB_CONNECTION=$cosmosConnectionString WEBSITE_RUN_FROM_ZIP=$releaseURL?$releaseSAS
